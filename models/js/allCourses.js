$(document).ready(function () {
    ...
    function get_course_table(page) {
        ...
        $.ajax({
            url: '../../models/controller/getAllCourses.php',
            type: 'GET',
            data: {
                page: page,
                id: id
    ...
    function manage_data_table(data) {
        ...
        $.each(data, function (key, value) {
            ...
            row += '<p id="onEnroll"><a href="enrollment.php?id=' + value.id + '&amp;token=' + value.tokenId + '" class="btn btn-primary btn-block" role="button">Enroll</a> </p>';
            row += '</div>';
            row += '</div>';
            row += '</div>';
        });
